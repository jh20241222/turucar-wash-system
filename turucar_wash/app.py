...
# (2026-09-07) 예전엔 나중에 추가하기로 미뤄뒀던 "카일이삼제스퍼"와, SK렌터카 3개 지점
# (광주상무/수원팔달/동작서부)을 추가 — 이제 이 소속 차량들도 밴드가 아니라 여기서
# 세차 현장 사진을 올린다. 이 set 하나만 바뀌면 정식 세차 오더(car_detail)뿐 아니라
# 완료 현황(wash_record) 사진 표시, "혼용 차량"으로 등록해서 들어가는 사진 업로드
# (mixed_car_register → car_detail로 이어지는 화면 포함)까지 전부 같은 기준을 쓰므로,
# 혼용 차량도 이 소속이면 자동으로 사진 업로드 화면이 뜬다 — 별도로 손댈 코드가 없다.
PHOTO_UPLOAD_ORGS = {
    "피플카 카셰어링",
    "휴맥스모빌리티",
    "카일이삼제스퍼",
