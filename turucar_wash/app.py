...
def _otoplug_session(car_org):
    ...
    try:
        ...
        pass
    return None
def _log_door_action(car_no, command, ok, message):
    """도어 제어 실행 기록: 누가(계정) 언제 어떤 차량에 무슨 명령을 내렸고 성공했는지 남긴다."""
    try:
        conn = get_wash_db()
        conn.execute(
            """
            INSERT INTO door_control_log (차량번호, command, username, role, ok, message, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (
                car_no, command,
                ...
                1 if ok else 0, str(message), now_kst().isoformat()
            )
        )
        conn.commit()
        conn.close()
    except Exception as log_err:
        print(f"[TuruWash] door_control_log 기록 실패: {log_err}")
def _door_result(car_no, command, ok, message, status=200):
    _log_door_action(car_no, command, ok, message)
    resp = jsonify({"ok": ok, "message": message})
...
def car_control(wash_id):
    ...
    if command not in ("lock", "unlock", "hazard"):
        return _door_result(None, command, False, "잘못된 명령", 400)
    conn = get_wash_db()
    car = conn.execute(
    ...
    conn.close()
    car_no = car["차량번호"] if car else None
    if not car:
        return _door_result(car_no, command, False, "차량 없음", 404)
    if not terminal:
        return _door_result(car_no, command, False, "단말기 정보 없음. 관리자에게 문의하세요.", 404)
    sess = _otoplug_session(car["차량소속"])
    if not sess:
        return _door_result(car_no, command, False, "오토플러그 로그인 실패", 500)
    url_map = {
    ...
    try:
        ...
        if result.get("smsIdx", 0) > 0:
            return _door_result(car_no, command, True, "명령 전송 완료")
        else:
            return _door_result(car_no, command, False, f"서버 거부: {result}")
    except Exception as e:
        return _door_result(car_no, command, False, f"통신 오류: {str(e)}")
...
def door_control_action():
    ...
    if not car_no:
        return _door_result(car_no, command, False, "차량번호를 입력해주세요.")
    if command not in ("lock", "unlock", "hazard"):
        return _door_result(car_no, command, False, "잘못된 명령")
    conn = get_wash_db()
    terminal = conn.execute(
    ...
    conn.close()
    if not terminal:
        return _door_result(car_no, command, False, f"'{car_no}' 단말기 정보 없음. 단말기 등록 여부를 확인하세요.")
    sess = _otoplug_session(terminal["차량소속"])
    if not sess:
        return _door_result(car_no, command, False, "오토플러그 로그인 실패")
    url_map = {
        "unlock": "https://maintenance.otoplug.com:40443/tl/doorunlock",
        "lock":   "https://maintenance.otoplug.com:40443/tl/doorlock",
        "hazard": "https://maintenance.otoplug.com:40443/tl/hazard",
...
# 도어 제어 로그 조회 (마스터 전용)
# =========================================================
DOOR_COMMAND_LABELS = {"lock": "잠금", "unlock": "잠금해제", "hazard": "비상등"}
@app.route("/door_control_log")
@login_required
def door_control_log_page():
    if not current_user.is_master:
        flash("❌ 마스터 계정만 볼 수 있습니다.")
        return redirect(url_for("dashboard"))
    car_no_filter = request.args.get("car_no", "").strip()
    username_filter = request.args.get("username", "").strip()
    conn = get_wash_db()
    query = "SELECT * FROM door_control_log WHERE 1=1"
    params = []
    if car_no_filter:
        query += " AND 차량번호 LIKE ?"
        params.append(f"%{car_no_filter}%")
    if username_filter:
    ...
    conn.close()
    return render_template(
        "door_control_log.html",
        logs=logs,
        cmd_label=DOOR_COMMAND_LABELS,
        filter_car_no=car_no_filter,
